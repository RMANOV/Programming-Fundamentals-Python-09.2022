...
# Get first input - contests and passwords - until "end of contests" - save in dictionary
contests = {}
command = input()
...
# Get second input - contests, passwords, users and points - until "end of submissions" - save in dictionary
users_points = {}
command2 = input()
while not command2 == "end of submissions":
    contest, password, user, points = command2.split("=>")
    points = int(points)
    if contest in contests and password == contests[contest]:
        if user not in users_points:
            users_points[user] = {}
        if contest not in users_points[user]:
            users_points[user][contest] = points
        if points > users_points[user][contest]:
            users_points[user][contest] = points
    command2 = input()

# Print the best user in the format "Best candidate is {user} with total {total points} points."
print(f"Best candidate is {max(users_points, key=lambda x: sum(users_points[x].values()))} with total {sum(users_points[max(users_points, key=lambda x: sum(users_points[x].values()))].values())} points.")
print("Ranking:")
for user in sorted(users_points):
    print(user)
    for contest in sorted(users_points[user], key=lambda x: -users_points[user][x]):
        print(f"#  {contest} -> {users_points[user][contest]}")
...
# contests_passwords = {
#     contest: password
#     for contest, password in [input().split(":") for _ in range(100)]
#     if input() != "end of contests"
# }
# user_contests_points = {
#     user: {
#         contest: int(points)
#         for contest, password, user, points in [
#             input().split("=>") for _ in range(int(input()))
#         ]
#         if contest in contests_passwords and password == contests_passwords[contest]
#     }
#     for user in [input().split("=>")[2] for _ in range(int(input()))]
#     if input() != "end of submissions"
# }

# # calculate total points for each user and find the best user
# best_user = max(
#     user_contests_points, key=lambda user: sum(user_contests_points[user].values())
# )

# # print the best user
# print(
#     f"Best candidate is {best_user} with total {sum(user_contests_points[best_user].values())} points."
# )
...
#                         user_contests_points[user].items(),
#                         key=lambda contest_points: -contest_points[1],
#                     )
#                 ]
#             )
#             for user in sorted(user_contests_points)
#         ]
#     )
# )
