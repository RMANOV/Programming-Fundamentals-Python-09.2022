...
# def switch(old_string, new_string, initial_string):
...
def main():
    ...
    while not command[0] == "Travel":
        if command[0] == "Add Stop":
            initial_string = add_stop(
                int(command[1]), command[2], initial_string)
            print(initial_string)
        elif command[0] == "Remove Stop":
            initial_string = remove_stop(
                int(command[1]), int(command[2]), initial_string)
            print(initial_string)
        elif command[0] == "Switch":
            initial_string = switch(command[1], command[2], initial_string)
            print(initial_string)
        command = get_command()
...
def add_stop(index, string, initial_string):
    if index in range(len(initial_string)):
        initial_string = initial_string[:index] + \
            string + initial_string[index:]
    return initial_string


def remove_stop(start_index, end_index, initial_string):
    if start_index in range(len(initial_string)) and end_index in range(len(initial_string)):
        initial_string = initial_string[:start_index] + \
            initial_string[end_index + 1:]
    return initial_string
