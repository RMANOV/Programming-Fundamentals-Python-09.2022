...
'
' HUNTER7 Macro
' Macro recorded 26.6.2022 by r.manov
'
...
'If Cells(R + 1, 8).Visible Then Cells(R + 1, 8).Select
...
NEXTCORRECTION:
If greencol - (BK - 1) >= 3 Then GoTo ENDINGS
        AVERAGEs = 0
        PROMOq = 0
    For Each CELL In Selection
    'IZ4ISLIAVA SREDNATA OT TRI SDEDMICI PREDI PREDHODNATA
       ...
       'If CELL.Interior.ColorIndex = 0 Or CELL.Interior.ColorIndex = 2 Then
        'AVERAGEs = AVERAGEs + CELL.Offset(0, -1).Value + CELL.Offset(0, -2).Value + CELL.Offset(0, -3).Value + CELL.Offset(0, -4).Value + CELL.Offset(0, -5).Value
        'PROMOq = PROMOq + 1
       ' ActiveCell.Offset(0, -1).Select
        'End If
       ' Next I
    ...
    'IZ4ISLIAVA JALTITE KLETKI I PARVA PROMOCIONALANA
        If CELL.Interior.ColorIndex = 36 Then
        For J = 1 To 4
            If CELL.Column < 8 Then GoTo NEXT2
            If Not CELL.Offset(0, -J).Interior.ColorIndex = 36 Or CELL.Offset(0, -J).Interior.ColorIndex = 39 Then
            If CELL.Offset(0, -J).Column = 2 Then GoTo NEXT1
            End If
            AVERAGEs = AVERAGEs + CELL.Offset(0, -J).Value
            PROMOq = PROMOq + 1
            'PROMOq = PROMOq - 1
NEXT1:
            Next J
            AVERAGEs = AVERAGEs / PROMOq
NEXT2:
        


        'AVERAGEs = (AVERAGEs + CELL.Offset(0, -1).Value + CELL.Offset(0, -2).Value + CELL.Offset(0, -3).Value + CELL.Offset(0, -4).Value + CELL.Offset(0, -5).Value) / 5
...
For I = 0 To 4
If ActiveCell.Columns <= greencol Then GoTo ENDINGS
...
control_koef = (srednaprognoza / AVERAGEs)


If control_koef > 1 Then

Cells(br, greencol + 200).Select
ActiveCell.Value = control_koef
ActiveCell.Offset(0, 1).Select
ActiveCell.FormulaR1C1 = "=SQRT(SQRT(SQRT(RC[-1])))"
control_koef = ActiveCell.Value
Cells(br, greencol).Select
End If

'If control_koef<1 then control_koef = (srednaprognoza/AVERAGEs)


If control_koef > 1.7 Then control_koef = 1.7
If control_koef < 0.3 Then control_koef = 0.3

'zapametiava averages za CONTRAHUNTER v kolona na 300 KLETKI
'Cells(BR, 300).Value = AVERAGEs
'vra6ta se na kletka BR, BK
'Cells(BR, BK).Select
...
'SRAVNIAVA PROGNOZATA NA VERIGATA S AVERAGE0 I AVERAGE1
If PROGNOZAVERIGA > AVERAGE0 And PROGNOZAVERIGA > AVERAGE1 Then
MsgBox ("PROGNOZATA NA VERIGATA E PO GOLIAMA: " & PROGNOZAVERIGA)
AVERAGE0 = AVERAGE0 + PROGNOZAVERIGA
End If
...
If PROGNOZAVERIGA > AVERAGE0 And PROGNOZAVERIGA > AVERAGE1 Then MsgBox ("PROGNOZATA NA VERIGATA E PO GOLIAMA: " & PROGNOZAVERIGA)
...
'SRAVNIAVA PROGNOZATA NA VERIGATA S AVERAGE0
If PROGNOZAVERIGA > AVERAGE0 Or PROGNOZAVERIGA > AVERAGE1 Then MsgBox ("PROGNOZATA NA VERIGATA E PO GOLIAMA: " & PROGNOZAVERIGA)
